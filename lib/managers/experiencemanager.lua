...
function ExperienceManager:load(data)
	...
	if state then
		...
		if not (self._global.next_level_data and tweak_data.experience_manager.levels[self._global.level + 1]) or self._global.next_level_data.points ~= tweak_data.experience_manager.levels[self._global.level + 1].points then
			self:_set_next_level_data(self._global.level + 1)
		end
	end
