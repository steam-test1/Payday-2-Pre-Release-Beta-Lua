...
function ExperienceManager:_set_next_level_data_points(value)
	self._global.next_level_data.points = Application:digest_value(value, true)
end
...
function ExperienceManager:load(data)
	...
	if state then
		...
		if not (self._global.next_level_data and tweak_data.experience_manager.levels[self:current_level() + 1]) or self:next_level_data_points() ~= tweak_data.experience_manager.levels[self:current_level() + 1].points then
			self:_set_next_level_data(self:current_level() + 1)
		end
	end
