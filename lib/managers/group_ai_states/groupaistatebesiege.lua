...
function GroupAIStateBesiege:_spawn_in_group(spawn_group, spawn_group_type, grp_objective, ai_task)
	...
	while i <= #valid_unit_types do
		...
		if i <= #valid_unit_types and 0 < wanted_nr_units and spawn_entry.amount_min and 0 < spawn_entry.amount_min and (not spawn_entry.amount_max or 0 < spawn_entry.amount_max) then
...
function GroupAIStateBesiege:_upd_reenforce_tasks()
	...
	while 0 < i do
		...
		if force_required then
			...
			if 0 < undershot and not self._task_data.regroup.active and self._task_data.assault.phase ~= "fade" and t > self._task_data.reenforce.next_dispatch_t and self:is_area_safe(task_data.target_area) then
				local used_event
				if task_data.use_spawn_event then
					task_data.use_spawn_event = false
...
function GroupAIStateBesiege:_set_recon_objective_to_group(group)
	...
	if not (target_area.loot or target_area.hostages) or not current_objective.moving_out and current_objective.moved_in and group.in_place_t and self._t - group.in_place_t > 15 then
		local recon_area
		local to_search_areas = {
	...
	if obstructed_area then
		...
		elseif not current_objective.moving_out then
			...
			elseif not phase_is_anticipation and not current_objective.open_fire then
				open_fire = true
			elseif not phase_is_anticipation and group.in_place_t and (not (not group.is_chasing and tactics_map and tactics_map.ranged_fire) or self._t - group.in_place_t > 15) then
				push = true
