...
function GroupAIStateBase:set_AI_enabled(state)
	...
	if Network:is_server() then
		for u_key, u_data in pairs(managers.enemy:all_enemies()) do
			...
			if not (not state or is_active) or not state and is_active then
...
function GroupAIStateBase:_debug_draw_drama(t)
	...
	if t > drama_hist[#drama_hist][2] then
		if #drama_hist == 1 then
			...
			if 1 < #drama_hist and math.abs(tan1 - tan2) < 0.5 then
	...
	if t > last_entry[2] then
		...
		if 1 < pop_hist_size and last_entry[1] == police_force and pop_hist[pop_hist_size - 1][1] == police_force then
			last_entry[2] = t
		else
...
function GroupAIStateBase:spawn_one_teamAI(is_drop_in, char_name, spawn_on_unit)
	if not (not Global.criminal_team_AI_disabled and self._ai_enabled) or not managers.criminals:character_taken_by_name(char_name) and managers.criminals:nr_AI_criminals() >= managers.criminals.MAX_NR_TEAM_AI then
		return
	end
...
function GroupAIStateBase:set_importance_weight(u_key, wgt_report)
	...
	for i_dis_rep = #wgt_report - 1, 1, -2 do
		...
		local i_imp = #imp_dis
		while true do
			if not (0 < i_imp) or c_dis >= imp_dis[i_imp] then
				break
			end
			i_imp = i_imp - 1
		end
...
function GroupAIStateBase:chk_say_enemy_chatter(unit, unit_pos, chatter_type)
	...
	if group_requirement and 1 < group_requirement then
...
function GroupAIStateBase:_merge_coarse_path_by_area(coarse_path)
	...
	local last_area
	while 0 < i_nav_seg do
		local nav_seg = coarse_path[i_nav_seg][1]
		local area = self:get_area_from_nav_seg_id(nav_seg)
		if last_area and last_area == area and 2 < #coarse_path then
...
function GroupAIStateBase:trim_coarse_path_to_areas(coarse_path)
	local all_areas = self._area_data
	local i = 1
	while 3 <= #coarse_path and i < #coarse_path do
