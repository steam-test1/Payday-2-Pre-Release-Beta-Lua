...
function CopActionWalk:update(t)
	...
	if move_dir and not self._expired then
		...
		if not (self._walk_anim_velocities[pose] and self._walk_anim_velocities[pose][self._stance.name] and self._walk_anim_velocities[pose][self._stance.name][variant]) or not self._walk_anim_velocities[pose][self._stance.name][variant][wanted_walk_dir] then
...
function CopActionWalk:_adjust_move_anim(side, speed)
	...
	if move_side and (side == move_side or self._matching_walk_anims[side][move_side]) then
		local seg_rel_t = self._machine:segment_relative_time(idstr_base)
		if not (self._walk_anim_lengths[anim_data.pose] and self._walk_anim_lengths[anim_data.pose][self._stance.name] and self._walk_anim_lengths[anim_data.pose][self._stance.name][speed]) or not self._walk_anim_lengths[anim_data.pose][self._stance.name][speed][side] then
...
function CopActionWalk:stop(pos)
	...
	local s_path = self._simplified_path
	if 0 < #s_path and not s_path[#s_path].x then
...
function CopActionWalk:_upd_walk_turn_first_frame(t)
	...
	local seg_rel_t = self._machine:segment_relative_time(idstr_base)
	local left_foot = seg_rel_t < 0.25 or 0.75 < seg_rel_t
	local anim = "walk_turn_" .. (0 < right_dot and "r" or "l") .. "_" .. (left_foot and "l" or "r") .. "f"
	local result = self._ext_movement:play_redirect(anim)
	if result then
