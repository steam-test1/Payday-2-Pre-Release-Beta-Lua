...
function CopLogicIdle._upd_scan(data, my_data)
	...
	if not (my_data.stare_pos and my_data.next_scan_t) or data.t < my_data.next_scan_t then
...
function CopLogicIdle.on_new_objective(data, old_objective)
	...
	if new_objective then
		...
		elseif not (not new_objective.action and data.attention_obj) or not (data.attention_obj.reaction >= AIAttentionObject.REACT_AIM) then
			CopLogicBase._exit(data.unit, "idle")
		else
